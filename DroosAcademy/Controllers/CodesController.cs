...
namespace DroosAcademy.Controllers
{
    ...
    public class CodesController : ControllerBase
    {
        ...
        {
            Task<Code> task = createCode(lectureId);
            Code code = await task;
            Response r = new Response();
            r.message = "تم تنفيذ العملية بنجاح";
            r.data = code;
            return Ok(r);
        ...
        public async Task<ActionResult<List<Code>>> getListOfCodes(CodeNumbers codenumber)
        {
            ...
            {
                Task<Code> task = createCode(codenumber.lectureId);
                Code code = await task;
                c.Add(code);
            }
            Response r = new Response();
        ...
        [HttpGet("getLectureNumberOfActivatedCodes/{id}")]
        [Authorize(Roles = "admin,teacher")]
        public async Task<ActionResult<int>> getLectureNumberOfActivatedCodes(int id)
        {
            List<Code> c = await _context.Codes.Where(e => e.LectureId == id && e.Status == true).ToListAsync();
            Response r = new Response();
            r.message = "تم تنفيذ العملية بنجاح";
            r.data = c.Count;
            return Ok(r);
        ...
        [AllowAnonymous]
        public async Task<ActionResult<Response>> activateLecture(StudentCode s)
        {
            Code code = await _context.Codes.FirstOrDefaultAsync(e => e.Name == s.Name);
            ...
            if(code != null)
            {
                if (code.Status != true)
                {
                    ...
                    Teacher teacher = await _context.Teachers.FindAsync(code.TeacherId);
                    ...
                    if (teacher.BalanceForPlatform == null) teacher.BalanceForPlatform = 0;
                    if (teacher.BalanceForTeacher == null) teacher.BalanceForTeacher = 0;
                    double balanceForTeacher = (double)lecture.Cost * (100 - (double)teacher.Percentage) / 100.0;
                    double BalanceForPlatform = (double)lecture.Cost * (double)teacher.Percentage / 100.0;
                    teacher.BalanceForTeacher += (int)Math.Floor(balanceForTeacher);
                    teacher.BalanceForPlatform += (int)Math.Ceiling(BalanceForPlatform);
            ...
            v = new Response();
            v.message = "تم استخدام هذا الكود من قبل";
            v.data = null;
            return Ok(v);
        }
        ...
        private async  Task<Code> createCode(int lectureId)
        {
